...
let
  wans =
    if args ? wans then args.wans else abort "radvd: args.wans must be defined (multiwan mandatory)";

  wan0 =
    if (builtins.length wans) > 0 then builtins.elemAt wans 0 else abort "radvd: args.wans is empty";

  prefixFile = wan0.publicPrefixFile or (abort "radvd: first WAN is missing publicPrefixFile");
