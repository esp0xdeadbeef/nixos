...
let
  prefixFile = args.publicPrefixFile or "/run/secrets/subnet-ipv6";

  raIfaces =
    args.raIfaces or
    map (l: l.iface) (lib.filter (l: l.ra6 or false) args.lans);
  ...
  genScript = pkgs.writeShellScript "v6-ra-generate" ''
    ...
    for IFACE in $ALLOWED_IFACES; do
      ...
      ${pkgs.iproute2}/bin/ip link show "$IFACE" | ${pkgs.gnugrep}/bin/grep -q "UP" || {
