...
# Temporary ALFA USB AP container inside the cobalt router VM. It owns the
# USB WiFi interface (moved in via --network-interface) and a veth onto the
# LAN trunk, and bridges two SSIDs (clients VLAN 30, clients-vpn VLAN 31)
# back onto that trunk so they behave like the wired access containers.
let
  wifiIf = "wlan0";
in
{
