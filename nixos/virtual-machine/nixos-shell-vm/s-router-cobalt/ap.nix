...
# through via qemu-xhci and appears as wlan0 in the VM's own netns, so the
# wireless stack and firmware are exercised here before the interface is
# moved into the dedicated `ap` container. Each SSID bridges onto the
# existing container bridge (clients VLAN 30, clients-vpn VLAN 31).
let
  wifiIf = "wlan0";
in
{
