...
let
  s-router-clab-render-live = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      lab_dir="$labs_repo/labs/lab-s-sigma/s-router-test-three-site"
      ...
      flock /run/s-router-clab-render-live.lock machinectl shell -q "$container_name" /run/current-system/sw/bin/bash -lc "
        ...
        bash '$container_work_dir/ensure-clab-tooling-image.sh'
        ...
        containerlab deploy -t '$container_work_dir/fabric.clab.yml' -d --reconfigure
...
in
{
  ...
  systemd.services.s-router-clab-render-live = {
    ...
    after = [
      "container@s-router-clab-container.service"
