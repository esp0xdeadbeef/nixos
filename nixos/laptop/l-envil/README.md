...
The disko config is `nixos/laptop/l-envil/disko/build_disko.nix`.
...
nix --extra-experimental-features "nix-command flakes" run github:nix-community/disko/latest -- \
  ...
  /home/nixos/github/nixos/nixos/laptop/l-envil/disko/build_disko.nix
```

## SOPS keys

Create host age keys before install and add the public keys to `.sops.yaml`.
Then re-encrypt the l-envil secrets.
...
```bash
nixos-install --no-root-passwd --impure --flake path:/home/nixos/github/nixos#l-envil
```

If the repo was copied as root instead:

```bash
nixos-install --no-root-passwd --impure --flake path:/root/github/nixos#l-envil
```
