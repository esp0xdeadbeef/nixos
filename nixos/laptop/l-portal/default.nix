...
in
{
  ...
  environment.etc."systemd/system-sleep/x13s-hibernate-device-workarounds".source =
    pkgs.writeShellScript "x13s-hibernate-device-workarounds" ''
      set -eu

      log() {
        printf '<6>x13s-hibernate-device-workarounds: %s\n' "$*" > /dev/kmsg || true
      }
      ...
      bind_if_needed() {
        ...
        device="$2"
        module="$3"
        if [ -e "/sys/bus/$bus/devices/$device/driver" ]; then
          log "$device already bound on $bus"
          return
        fi
        if [ -e "/sys/bus/$bus/drivers/$module/bind" ]; then
          log "binding $device to $module"
          echo "$device" > "/sys/bus/$bus/drivers/$module/bind" || log "bind failed for $device"
        fi
      }
      ...
      unbind_if_bound() {
        ...
        device="$2"
        module="$3"
        if [ -e "/sys/bus/$bus/devices/$device/driver/unbind" ]; then
          log "unbinding $device from $module"
          echo "$device" > "/sys/bus/$bus/devices/$device/driver/unbind" || log "unbind failed for $device"
        else
          log "$device is not bound to $module"
        fi
      }

      bind_platform_path_if_needed() {
        path="$1"
        module="$2"
        device="''${path##*/}"
        if [ -e "$path/driver" ]; then
          log "$device already bound on platform"
          return
        fi
        if [ -e "/sys/bus/platform/drivers/$module/bind" ]; then
          log "binding $device to $module"
          echo "$device" > "/sys/bus/platform/drivers/$module/bind" || log "bind failed for $device"
        fi
      }

      unbind_platform_path_if_bound() {
        path="$1"
        module="$2"
        device="''${path##*/}"
        if [ -e "$path/driver/unbind" ]; then
          log "unbinding $device from $module"
          echo "$device" > "$path/driver/unbind" || log "unbind failed for $device"
        else
          log "$device is not bound to $module"
        fi
      }

      # Disabled X13s hibernate workaround: unbinding ELAN I2C HID and
      # xhci-hcd.8.auto avoided earlier restore errors, but made later
      # systemd hibernate attempts hang before the kernel created an image.
      # Keep this hook as a log marker while testing the official systemd
      # freezer workaround; do not unbind devices here until the pre-image hang
      # is understood.
      case "''${1:-} ''${2:-}" in
        "pre hibernate"|"pre hybrid-sleep")
          log "pre hibernate: ELAN I2C HID/xHCI unbind workaround disabled"
          ;;
        "post hibernate"|"post hybrid-sleep")
          log "post hibernate: ELAN I2C HID/xHCI rebind workaround disabled"
          ;;
      esac
    '';
