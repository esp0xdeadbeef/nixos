...
{ inputs, outPath, ... }:
{
  ...
  legcord-unstable-overwrite = final: _prev: {
    # Explicit overwrite: stable legcord currently pulls a pnpm package that
    # Nixpkgs marks insecure, so pkgs.legcord follows unstable.
    legcord =
      builtins.trace
        "WARNING: explicit overlay overwrite: pkgs.legcord = pkgs.unstable.legcord because stable legcord pulls pnpm-10.29.2, which Nixpkgs marks insecure."
        ...
        patchedLib =
          ...
          // {
            mkRemovedOptionModule =
              ...
              if optionName == [ "method" ] then
                _moduleArgs: { }
              else
                lib.mkRemovedOptionModule optionName replacementInstructions;
          };
