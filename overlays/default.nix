# This file defines overlays
{
  ...
  legcord-unstable-overwrite = final: _prev: {
    # Explicit overwrite: legcord still asks for pnpm_10_29_2, which Nixpkgs
    # marks insecure. Keep legcord on unstable, but force its pnpm input to the
    # patched pnpm_10 from the active package set.
    legcord =
      builtins.trace
      "WARNING: local legcord overlay is active: pkgs.legcord = pkgs.unstable.legcord with pnpm_10_29_2 overridden to patched pkgs.pnpm_10 (${final.pnpm_10.version}) because upstream legcord still asks for insecure pnpm-10.29.2."
      (final.unstable.legcord.override {
        pnpm_10_29_2 = final.pnpm_10;
      });
  };

  impermanence-module = final: _prev: {
    impermanenceNixosModule = args @ {
      config,
      lib,
      pkgs,
      ...
