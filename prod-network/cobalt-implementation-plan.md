...
```
   ...
   l-portal USB (port 2) ─── VLAN 2 untagged
```
...
- Switch L2 validated with tcpdump:
  - l-portal untagged → trunk sees **VLAN 2** tagged ✅
  - ISP untagged → l-envil USB sees **VLAN 300** tagged ✅
  - trunk shows **no VLAN 300** ✅
- l-portal on VLAN 2 gets a DHCP lease and reaches the internet (pending cobalt VM).
