...
let
  ...
  # FS-370: access-clients-vpn is a multi-uplink access (onyx, opal), so the
  # forwarding model emits one policy<->upstream-selector lane per permitted
  # uplink. The inventory binds one port per lane.
        ...
        uplinks = {
          upstream-core = {
            ...
            ipv4 = {
              ...
              dhcpClient = "udhcpc";
              sendHostname = false;
              # Request the DNS option (6) from the provider so the resolver
              # can bootstrap its root trust anchor off the provider DNS.
              dhcpOptions = [ "dns" ];
