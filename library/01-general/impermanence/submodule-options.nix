...
let
  ...
  # NixOS 26.05 `lib.mkRemovedOptionModule` declares the removed option with an
  # `apply = throw`, which fires even for undefined options whenever a
  # submodule config is forced. Declare removed options as hidden,
  # null-defaulted options guarded by an assertion instead, so the helpful
  # error survives without the spurious throw.
  mkDeprecatedOption = name: message:
    ({ config, ... }: {
