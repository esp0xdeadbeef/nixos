    ...
    let
      normalize48 =
        addr:
          ...
          in
          if plen != "48" then null
          else lib.concatStringsSep ":" hextets;
            ...
            lib.concatMap
              (ep:
                if ep ? addr6 && builtins.isString ep.addr6 then
                  let base = normalize48 ep.addr6;
                  in if base != null then [ base ] else []
                else
                  []
              )
      ...
      (_: l:
        if l.kind == "lan" && lib.length (l.members or []) == 1 then
          let
            ...
            gua64 =
              if delegatedV6 != null && vid != null
              then "${delegatedV6}:${toString vid}::/64"
              else null;

            guaAddr =
              if delegatedV6 != null && vid != null
              then "${delegatedV6}:${toString vid}::1/64"
              else null;
          in
          l // {
