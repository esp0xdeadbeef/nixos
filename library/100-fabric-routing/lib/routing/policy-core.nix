...
let
  ...
  stripCidr =
    s:
      if s == null then null
      else builtins.elemAt (lib.splitString "/" s) 0;
  ...
  setEp = l: n: ep:
    l // { endpoints = (l.endpoints or {}) // { "${n}" = ep; }; };
        ...
        let
          ...
          ep =
            if l == null then null
              ...
              let node = lib.head (l.members or []);
              in (l.endpoints or {}).${node} or null;
        in
        if ep != null && ep ? addr6Public then
          let ip = stripCidr ep.addr6Public;
          in if ip != null && lib.hasSuffix "::1" ip
             then [ "${builtins.replaceStrings [ "::1" ] [ "::" ] ip}/64" ]
             else [ "${ip}/64" ]
        else
          []
      )
      ...
      (_: l:
        ...
        then
          let
            ...
            coreRoutes4 =
              map (vid: { dst = tenant4Dst vid; via4 = via4toPolicy; }) tenantVids;

            coreRoutes6 =
              (map (vid: { dst = tenant6DstUla vid; via6 = via6toPolicy; }) tenantVids)
              ++ (map (pfx: { dst = pfx; via6 = via6toPolicy; }) tenantGuaPrefixes);
