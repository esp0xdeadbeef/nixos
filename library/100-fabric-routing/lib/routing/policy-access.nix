...
let
  ...
  tenantLanEpFor =
    accessNode: vid:
      ...
      in
      if l == null then null
      else (l.endpoints or {}).${accessNode} or null;

  # Convert "...::1/64" to "...::/64" (best-effort, deterministic for our synthesis)
  prefix64FromHostAddr =
    addr:
      let ip = stripCidr addr;
      in
      if ip == null then null
      else if lib.hasSuffix "::1" ip then "${builtins.replaceStrings [ "::1" ] [ "::" ] ip}/64"
      else "${ip}/64";
      ...
      (lname: l:
        if !(isPolicyAccess lname l) then
          ...
          let
            ...
            tenantGuaPrefix =
              if tenantLanEp != null && tenantLanEp ? addr6Public
              then prefix64FromHostAddr tenantLanEp.addr6Public
              else null;
            ...
            policyRoutes4 = [
              { dst = tenant4Dst vid; via4 = via4toAccess; }
            ];

            policyRoutes6 =
              [ { dst = tenant6DstUla vid; via6 = via6toAccess; } ]
