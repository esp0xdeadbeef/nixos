...
{ names, routes, parentIf }:

{ all, nodeName, linkName, iface }:
...
let
  ...
  routeSections =
    map routes.mkRouteSection
      ((iface.routes4 or [ ]) ++ (iface.routes6 or [ ]));

in
{
  netdevs =
    { "20-${vname}.netdev" = {
        netdevConfig = { Name = vname; Kind = "vlan"; };
        vlanConfig.Id = vlanId;
      };
    }
    // lib.optionalAttrs needsBridge {
      "20-${bname}.netdev" = {
        netdevConfig = { Name = bname; Kind = "bridge"; };
      };
    };
  ...
  networks =
    {
      ...
      "30-${networkOn}.network" = {
        ...
        networkConfig = {
          Address = lib.filter (x: x != null) [ iface.addr4 or null iface.addr6 or null ];
          IPForward = "yes";
          IPv6AcceptRA = "no";
        };
