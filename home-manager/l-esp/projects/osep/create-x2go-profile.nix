...
let
  updateX2GoSession = pkgs.writeShellScriptBin "update-x2go-session" ''
    ...
    autologin=true
    clipboard=both
    command="startxfce4 ; /usr/bin/xfconf-query -c xfwm4 -p /general/use_compositing -s false"
    defsndport=true
    directrdp=false
    directrdpsettings=
    directxdmcp=false
    ...
    echo $(lxc-attach osep-lxc -- bash -c "ip a s eth0 | sed -n 's/.*inet \([0-9\.]\+\)\/.*/\1/p' | tr -d '\n' ; echo  ; cat /etc/ssh/ssh_host_rsa_key.pub" | tr '\n' ' ' ) >> "$HOME/.ssh/known_hosts"
    echo "x2goclient --session='x2go-session-$ID' --hide" > "$ID_FILE"
    chmod +x "$ID_FILE"
    "$ID_FILE"
  '';
in
